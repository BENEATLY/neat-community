...
@Component({selector: 'app-plugin-option-modal', templateUrl: 'plugin-option.component.html'})


// Component Export Definition
export class PluginOptionModalComponent implements OnInit {
  ...
  constructor(public data: DataService, private http: HttpClient, private cookieService: CookieService, public appConfig: AppConfig, public modalService: ModalService, private snackBar: SnackBarService, public timezone: TimezoneService, public translate: TranslateService, public translation: TranslationService) {
    ...
    // Generate Title
    meta.title = translate.instant('common.plugin.option.edit', {'plugin': meta.plugin});
  ...
  submitAllowed() {
    ...
    // Validate Input
    return formLib.validateInput(this.translate, this.properties, this.acceptedList, this.currentList, this.existingList, this.optionalList, this.partUnique, this.activeToolTips);
  ...
  async submit() {
    ...
    // Construct Data Form
    let rawData = await formLib.constructSendData(this.appConfig.config, this.data.userData, this.properties, {'type': 'Edit'}, this.optionalList, this.timezone, this.snackBar, this.cookieService, this.http, this.fileData);

    // Convert to Plugin Option Format
    let optionData = {};
    optionData[this.meta.group] = rawData;

    // Update Plugin Option Values
    await dataLib.setPrivatePluginValues(this.appConfig.config, this.meta.plugin, optionData, this.snackBar, this.cookieService, this.http);
